...
const objectCounts=()=>{const observed=objects().filter(o=>o.source_record).length;return`${objects().length-observed} 个场景对象 · ${observed} 条照片检测记录`;};
...
const selectionMessage=()=>{const o=current();send({type:'panoptes:select',objectId:o?(Object.hasOwn(o,'scene_object_id')?o.scene_object_id:o.id):null});};
...
function validate(data){
  ...
  const ids=new Set();for(const o of data.objects){if(typeof o.id!=='string'||ids.has(o.id)||!Array.isArray(o.views)||o.views.some(v=>!Array.isArray(v.polygons)||v.polygons.some(p=>!Array.isArray(p)||p.length<3||!p.every(finitePoint)))||!Array.isArray(o.inventory_indices)||(o.plan!==null&&(!o.plan?.hull?.length||!o.plan.hull.every(finitePoint)||!finitePoint(o.plan.center))))throw Error('场景对象或平面数据无效');ids.add(o.id);}
  if(typeof data.scene_url!=='string'||!data.scene_url)throw Error('报告缺少场景地址');asset(data.scene_url);
...
window.addEventListener('message',event=>{
  ...
  if(event.data.type==='panoptes:selection'&&state.data){const id=event.data.objectId,o=objects().find(o=>(o.scene_object_id||o.id)===id);if(id===null||o)select(o?.id||null,true);}
...
function renderChips(){
  ...
  const strip=$('object-list'),selected=strip.querySelector('[aria-pressed="true"]');if(selected){const outer=strip.getBoundingClientRect(),inner=selected.getBoundingClientRect();if(inner.left<outer.left)strip.scrollLeft+=inner.left-outer.left-8;else if(inner.right>outer.right)strip.scrollLeft+=inner.right-outer.right+8;}
}
function renderPhoto(){
  ...
  shapes.sort((a,b)=>b.view.polygons.reduce((sum,p)=>sum+area(p),0)-a.view.polygons.reduce((sum,p)=>sum+area(p),0));
...
function renderPlans(){
  ...
  const sorted=objects().filter(o=>o.plan).sort((a,b)=>area(b.plan.hull)-area(a.plan.hull));
  ...
  const highlight=selected?.plan?`<g pointer-events="none"><polygon points="${selected.plan.hull.map(project).map(p=>p.join(',')).join(' ')}" fill="none" stroke="#bc571f" stroke-width="3"/>${selected.id!=='observed_floor'?`<circle cx="${project(selected.plan.center)[0]}" cy="${project(selected.plan.center)[1]}" r="12" fill="none" stroke="#bc571f" stroke-width="1"/>`:''}</g>`:'';
  const unavailable=selected&&!selected.plan?'<text x="300" y="52" text-anchor="middle" font-size="12" fill="#bc571f">当前物体没有可靠平面范围；原图证据仍可查看。</text>':'';
  ...
  $('cad-view').innerHTML=`<defs><pattern id="cad-hatch" width="8" height="8" patternUnits="userSpaceOnUse"><path d="M-2,2L2,-2M0,8L8,0M6,10L10,6" stroke="#d6dcca" stroke-width=".5"/></pattern></defs>${legend}${svgObjects(true)}${highlight}${dimensions}${unavailable}`;
  ...
  $('plan-view').innerHTML=`${legend}<g id="plan-content">${svgObjects(false)}${highlight}${cameras}</g>${unavailable}`;
  updatePan();
}
...
function renderSelected(){
  ...
  $('selected-source').textContent=o?(o.spatial?.status==='unavailable'?'已有照片证据 · 空间范围不可用':o.generation?.status==='not_generated'?'照片观测范围 · 尚未生成完整模型':o.source==='parametric'?'Blender 参数化资产':o.source==='observed'?'照片观测表面':'独立生成资产'):'';
  $('selected-mapping').textContent=o?(o.source_record?`来源照片 ${state.data.frames.find(f=>f.id===o.source_record.target_frame_id)?.label||o.source_record.target_frame_id} · 检测 ${o.source_record.candidate_id}`:o.inventory_indices.length?`已核对原检测记录：${o.inventory_indices.map(i=>'#'+i).join('、')}`:'与原检测 inventory 暂无可靠对应'):'';
  $('object-detail-name').textContent=o?name(o):'点击照片、3D 或平面中的对象';
  $('object-detail-copy').textContent=o?(o.spatial?.status==='unavailable'?'空间证据不足，保留原图中的物体选择。'+(o.spatial.reason||''):o.generation?.status==='not_generated'?'已定位照片中的可见表面，可查看观测范围；完整模型尚未生成，遮挡部分未知。':o.source==='parametric'?'当前外形为 Blender 拟合圆柱；下方保留原生成资产的历史摆放实验指标，实际尺寸尚未标定。':o.source==='observed'?'仅保留照片观测到的表面；未补全不可见范围。':`${o.views.length} 个输入视图提供分割证据。网格的遮挡背面由模型生成，实际尺寸尚未标定。`):'四个视图共享同一个对象选择。';
  const m=o?.metrics;
  $('object-metrics').innerHTML=m&&Number.isFinite(m.after_iou)?`${o.source==='parametric'?'<p class="empty-note">原生成资产摆放指标 · 历史实验</p>':''}<div class="metric-readout"><small>轮廓 IoU · 摆放前 → 后</small><b>${pct(m.before_iou)} <em>→</em> ${pct(m.after_iou)}</b></div><div class="metric-readout"><small>相对深度误差 · 前 → 后</small><b>${pct(m.before_depth)} <em>→</em> ${pct(m.after_depth)}</b></div>`:'';
...
function renderLegacy(){
  ...
  $('report-status').innerHTML=`<span class="status-pill"><i class="status-dot"></i><strong>原检测：${fails} 项未通过 · ${unknown} 项证据不足</strong></span><i class="divider"></i><span>${objectCounts()} · ${state.data.frames.length} 张照片</span><i class="divider"></i><span>新重建尺度尚未标定</span>`;
  $('counts').textContent=`${objectCounts()} / ${inventory.length} 条原检测记录`;
...
(async()=>{
  try{
    const response=await fetch('workcell-data.json');if(!response.ok)throw Error('报告数据加载失败：'+response.status);const data=await response.json();validate(data);state.data=data;
    const viewerURL=new URL('viewer.html',location.href);viewerURL.searchParams.set('scene',data.scene_url);viewerURL.searchParams.set('embedded','1');if($('model-viewer').src!==viewerURL.href)$('model-viewer').src=viewerURL.href;
...
const playgrounds={
  ...
  blender:{name:'工位重建与物体范围',url:'viewer.html?scene=workcell-scene.json',preview:'blender/parametric-frame_0003.png',description:'生成资产、Blender 参数化物体与照片观测范围在同一工位查看。点选物体联动原图、3D、高宽深与坐标轴；尚未生成或空间证据不足的物体保留独立状态。'},
  components:{name:'逐图检测复核',url:'viewer.html?scene=components/scene.json',preview:'components/preview.png',description:'查看各照片中的物体观测范围，在原图和 3D 中点选、查看包围框与三轴。两个按钮保留完整生成网格；其余区域来自分割和深度，尚未合并为跨视角实体。'}
};
let playgroundId='blender';
