...
function renderLegacy(){
  ...
  $('findings').innerHTML=findings.map(f=>`<article class="finding"><div class="finding-top"><h4>${esc(f.title||f.id)}</h4>${verdict(f.status)}</div><p>${esc(f.summary)}</p>${f.evidence||f.metrics?`<details><summary>查看保存的证据</summary>${f.evidence?.url?`<a href="${esc(asset(f.evidence.url))}">原始证据与限制记录 JSON ↗</a>`:''}<pre>${esc(JSON.stringify({evidence:f.evidence,metrics:f.metrics},null,2))}</pre></details>`:''}</article>`).join('');
  $('inventory-summary').textContent=`展开全部 ${inventory.length} 条原检测记录`;
  $('legacy-inventory').innerHTML=`<table><thead><tr><th>记录</th><th>原标签</th><th>来源视图</th><th>对应新场景</th></tr></thead><tbody>${inventory.map(i=>{const mapped=objects().find(o=>o.inventory_indices.includes(i.inv));return`<tr data-inv="${i.inv}"><td><button data-inv="${i.inv}" aria-label="选择检测记录${i.inv}">#${i.inv}</button></td><td>${esc(i.label)}</td><td>${esc(i.frame_id||i.frame_ids?.join('、')||'—')}</td><td>${mapped?`<button data-object-id="${esc(mapped.id)}">${esc(name(mapped))} ↗</button>`:'暂无可靠对应'}</td></tr>`;}).join('')}</tbody></table>`;
}
